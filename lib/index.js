...
addFav.addEventListener("click", function (){
  event.preventDefault();
  cityName = {location: favorite.value, api_key: sessionStorage.setItem("userKey") }
...
function currentElement(data) {
  var text = `<ul class="card"><li>${data.city}  <button type="button" class="btn" id="favorite" value=${data.city} >Add To Favorites</button></li><li>Country: ${data.country}</li><li>${data.currently.time}</li><li>Precipation: ${data.currently.preciporcentage} %</li></ul>`
